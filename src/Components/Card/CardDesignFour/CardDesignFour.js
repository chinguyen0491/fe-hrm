  ...
  return (
    <div>
        ...
        <div className={styles.contentBox}>
            <h6 className={`text-left mt-4 mb-3 text-uppercase ${styles.titleBox}`}>{title}</h6>
            <ul className={`p-0`}>
            {
                dataJob.filter((item, idx) => idx < 4).map((ele,index)=> {
                    return    <li className={styles.listContent} key={index}>
                                <Link className={styles.link} to={toSlug(ele)}>{ele}</Link>
                            </li>
                })
            }
            </ul>
            <Link className={styles.link}  to='/'>(Xem thêm)</Link>
        </div>
    </div> 
  )
}
